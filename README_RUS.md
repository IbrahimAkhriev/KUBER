...
1) <Тут будет описан либо миникуб либо подключение удаленно> Запустить окружение Kubernetes с памятью 4GB (если миникуб) / Подключиться к кластеру по ssh.
...
9) Обновить приложение (добавив новую зависимость в pom-файл), и пересобрать приложение со следующими стратегиями разверывания (замерить время переразвертывания приложения для каждого случая и отметить результаты в отчете):
   - пересоздание (recreate)
   - последовательное обновление (rolling) 

💡 [Нажми тут](https://forms.yandex.ru/cloud/6475c362e010db267d71e431/), **чтобы поделиться с нами обратной связью на этот проект**. Это анонимно и поможет команде Педаго сделать твоё обучение лучше.
